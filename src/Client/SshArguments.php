...
class SshArguments
{
    ...
    public function getCliArguments()
    {
        ...
        $valueFlags = array_map(
            function ($key, $value) {
                return "$key $value";
            },
            array_keys($valueFlags),
            $valueFlags
        );
    ...
    public function withMultiplexing(HostInterface $host)
    {
    ...
    private function generateControlPath(HostInterface $host)
    {
        ...
        $fullHost = ($host->getUser() ? $host->getUser() . '@' : '') . $host->getHostname();
        $connectionData = "$fullHost$port";
